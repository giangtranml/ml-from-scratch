...
class LogisticRegression:
    ...
    def _prepare_data(self, num_points=100):
        X1 = np.random.multivariate_normal([5, 6], [[5, 0], [0, 5]], num_points)
        X2 = np.random.multivariate_normal([14, 15], [[4, 0], [0, 4]], num_points)
        X3 = np.random.multivariate_normal([1, 2], [[3, 0], [0, 3]], num_points)
        y1 = np.zeros(shape=(num_points, 1))
        y2 = np.ones(shape=(num_points, 1))
        y3 = np.ones(shape=(num_points, 1))*2
